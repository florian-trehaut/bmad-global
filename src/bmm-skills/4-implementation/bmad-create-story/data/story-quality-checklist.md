...
#### **3.6 Acceptance Criteria Observability DISASTERS**

🚨 **CRITICAL: The #1 cause of "done but not working" stories is AC that describe internal mechanics instead of observable behavior.**

**User AC validation:**
- Does EVERY User AC describe something a non-developer can verify in the browser?
- Does every action have an observable post-action state? (redirect destination, UI element change, content displayed)
- Are persistence scenarios covered? (page refresh, session survival, state after browser restart)
- Is the complete UI state specified? (nav changes, button states, error messages)
- REJECT any User AC containing: hashing algorithms, database engines, cache stores, cookie flags, protocol names, framework internals

**Technical AC validation:**
- Does every Technical AC reference a specific NFR or architecture constraint?
- Is it verifiable via automated test (not just code review)?
- Is it in the right section? (implementation HOW → Dev Notes, verifiable WHAT → Technical AC)

**Separation validation:**
- Are User AC and Technical AC clearly separated into distinct sections?
- Is there any AC that mixes user behavior with implementation details? → SPLIT IT
- If the original epics file had a single "Acceptance Criteria" section → it MUST be split
