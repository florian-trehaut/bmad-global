...
Read `{MAIN_PROJECT_ROOT}/.claude/workflow-knowledge/project.md` (HALT if missing — see `~/.claude/skills/bmad-shared/knowledge-loading.md`). This file contains project-specific environment URLs, staging/production endpoints, and frontend-specific configuration.

### 4. Load stack knowledge (optional)

Read `{MAIN_PROJECT_ROOT}/.claude/workflow-knowledge/project.md` (HALT if missing — see `~/.claude/skills/bmad-shared/knowledge-loading.md`). This file contains the technology stack, test framework conventions, and frontend framework details needed to construct the right test invocations.

### 5. Load validation knowledge (optional)

Read `{MAIN_PROJECT_ROOT}/.claude/workflow-knowledge/project.md` (HALT if missing — see `~/.claude/skills/bmad-shared/knowledge-loading.md`). This file is generated by `bmad-knowledge-bootstrap` and contains project-specific validation tooling, test discovery patterns, and stack-specific notes (WASM hydration, Tauri limitations, etc.).
