...
### 3. Load project knowledge (REQUIRED — single Read per M17 / TAC-14)
...
This single Read covers ALL three concerns previously split across §3 (environment config), §4 (stack knowledge), §5 (validation tooling) :

- **Environment config** : project-specific environment URLs, staging/production endpoints, frontend-specific configuration
- **Stack knowledge** : technology stack, test framework conventions, frontend framework details needed to construct the right test invocations
- **Validation knowledge** (generated by `bmad-knowledge-bootstrap`) : project-specific validation tooling, test discovery patterns, stack-specific notes (WASM hydration, Tauri limitations, etc.)
...
**Dedup rationale (M17 / TAC-14)** : the prior §3+§4+§5 pattern executed three identical Reads of the same knowledge file in a row, which inflated INIT context. Per `standalone-auto-flow-unification.md` §RevS-5, post-refactor `grep -c` returns 2 occurrences (1 Read at this section + 1 reference in the CHK-INIT receipt example) — verified against the validator-frontend scope.
