...
- Skip user selection — read `ISSUE_IDENTIFIER`, `ENVIRONMENT` from `task_contract` (HALT TAC-28 if missing)
- Read VM and DoD from `task_contract.input_artifacts[type=document]`
...
"Which environment should I validate against? **[T]est** (default — automated, headless) / **[L]ocal** (running the application binary, manual verification possible)"

- If no response or 'T' — `ENVIRONMENT = test`
