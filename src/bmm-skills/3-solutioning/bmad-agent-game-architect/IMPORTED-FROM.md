...
bmad-agent-game-architect skill (Cloud Dragonborn).
-->
...
|---|---|---|
| `src/agents/gds-agent-game-architect/SKILL.md` | `src/bmm-skills/3-solutioning/bmad-agent-game-architect/SKILL.md` | DIFF 1, DIFF 2, frontmatter rename, `disable-model-invocation` flag added, JIT-load domain stack inline (per Task 16 / Guardrail #7) |
| `src/agents/gds-agent-game-architect/customize.toml` | `src/bmm-skills/3-solutioning/bmad-agent-game-architect/customize.toml` | `[agent]` table verbatim (persona / role / identity / communication_style / principles preserved per Guardrail #4); menu adapted per TD-6 |
...
| Upstream | Our fork |
|---|---|
| `name: gds-agent-game-architect` | `name: bmad-agent-game-architect` |
...
|---|---|---|---|
| `GA` | `gds-game-architecture` | NOT present | Redirect → `bmad-create-architecture` (closest bmm equivalent — produces architecture artefact; game-specific adaptations come via the domain stack loaded in Step 5) |
| `PC` | `gds-generate-project-context` | NOT present | Redirect → `bmad-knowledge-bootstrap` (closest bmm equivalent — produces project knowledge files) |
| `CC` | `gds-correct-course` | EXISTS in our fork as `bmad-correct-course` (under `4-implementation/`) | Direct mapping → `bmad-correct-course` |
| `IR` | `gds-check-implementation-readiness` | NOT present | Redirect → `bmad-check-readiness` (closest bmm equivalent) |

**Rationale:** Per Guardrail #4 of the originating story, the BMGD persona content (role / identity / communication_style / principles) is preserved verbatim from SHA `9dcd1253`. The menu, however, is configuration — not persona — and the upstream menu items target skills that do not exist in this fork. Redirecting to closest bmm equivalents is the minimum-change adaptation that keeps the menu functional. Per-item decisions are documented as TOML comments above each `[[agent.menu]]` block in `customize.toml`.
...
curl -s "https://raw.githubusercontent.com/bmad-code-org/bmad-module-game-dev-studio/9dcd1253/src/agents/gds-agent-game-architect/SKILL.md" -o /tmp/upstream-skill.md
