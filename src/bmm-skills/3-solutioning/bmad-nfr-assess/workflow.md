...
Read `{MAIN_PROJECT_ROOT}/.claude/workflow-knowledge/project.md` (HALT if missing — see `~/.claude/skills/bmad-shared/core/knowledge-loading.md`). It provides the technology stack, service list, frameworks, ORMs, and architecture patterns — essential context for knowing what to scan and where.
