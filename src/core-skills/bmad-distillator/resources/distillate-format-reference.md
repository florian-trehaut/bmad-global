...
**Brief says:**
```
bmad-knowledge-bootstrap must always be included as a base skill in every bundle
```

**Discovery notes say:**
```
bmad-knowledge-bootstrap must always be included as a base skill in every bundle/install
(solves bootstrapping problem)
```

**Distillate keeps the more contextual version:**
```
- bmad-knowledge-bootstrap: always included as base skill in every bundle (solves bootstrapping)
```
...
- Three layers: self-describing plugins (bmad-manifest.json), cross-platform install via Vercel skills CLI (MIT), runtime registration via bmad-knowledge-bootstrap skill
...
- bmad-knowledge-bootstrap: global skill scanning installed bmad-manifest.json files, registering capabilities, configuring project settings; always included as base skill in every bundle (solves bootstrapping)
...
- bmad-knowledge-bootstrap discovers/registers all plugins from manifests; clear errors for malformed manifests
...
## Scope
- In: manifest spec, bmad-knowledge-bootstrap, bmad-update, Vercel CLI integration, NPX installer, zip bundles, migration path
...
- CI/CD integration: bmad-knowledge-bootstrap as pipeline one-liner increases stickiness
