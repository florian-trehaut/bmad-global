...
The following knowledge files are available in `{MAIN_PROJECT_ROOT}/.claude/workflow-knowledge/` and are loaded by all bmad-\* workflow skills via the protocol in `~/.claude/skills/bmad-shared/knowledge-loading.md`. They are generated by `/bmad-knowledge-bootstrap`.
